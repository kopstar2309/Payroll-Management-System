<?php 
	...
	if($_REQUEST['type'] == 1) $heading = "Admin User";
	if($_REQUEST['type'] == 2) $heading = "Emnployee";
	global $SERVER_PATH;
?>
<script>
		...
		<div class="contact">
			...
			<form name="frm_user" action="lib/user.php" method="post">
				<div class="static">
					...
					?>
					  <tr>
						<td style="text-align:center; font-weight:bold;"><?=$data[user_id]?></td>
						<td><img src="<?=$SERVER_PATH.'uploads/'.$data[user_image]?>" style="heigh:50px; width:50px"></td>
						<td><?=$data[user_name]?></td>
						<td><?=$data[user_mobile]?></td>
						<td><?=$data[user_email]?></td>
						<td><?=$data[user_dob]?></td>
						<td style="text-align:center"><a href="record.php?user_id=<?php echo $data[user_id] ?>">Company Record</a> | <a href="user.php?user_id=<?php echo $data[user_id] ?>">Edit</a> | <a href="Javascript:delete_user(<?=$data[user_id]?>)">Delete</a> </td>
					  </tr>
					<?php } ?>
					</tbody>
					</table>
